...
import com.hndw.smartlibrary.until.core.GPS;
...
public class GPSConverterUtils {
    ...
    public static GPS gcj2ToBD09(double glat, double glon) {
        double x = glon;
        double y = glat;
    ...
    public static GPS bd09ToGCJ2(double glat, double glon) {
    ...
    public static GPS bd09ToWGS(double glat, double glon) {
        GPS gps = bd09ToGCJ2(glat, glon);
        return gcj2ToWGS(gps.getWgLat(), gps.getWgLon());
    }

    //BD09=>WGS84 百度坐标系=>地球坐标系(精确)
    public static GPS bd09ToWGSExactly(double glat, double glon) {
        GPS gps = bd09ToGCJ2(glat, glon);
        return gcj2ToWGSExactly(gps.getWgLat(), gps.getWgLon());
    }

    // WGS84=》BD09   地球坐标系=>百度坐标系
    public static GPS wgsToBD09(double wgLat, double wgLon) {
        GPS gps = wgsToGCJ(wgLat, wgLon);
        return gcj2ToBD09(gps.getWgLat(), gps.getWgLon());
    }

    // WGS84=》GCJ02   地球坐标系=>火星坐标系
    public static GPS wgsToGCJ(double wgLat, double wgLon) {
        double[] latlon = new double[2];
    ...
    public static GPS gcj2ToWGS(double glat, double glon) {
    ...
    public static GPS gcj2ToWGSExactly(double gcjLat, double gcjLon) {
        double initDelta = 0.011;
        double threshold = 0.00000000095;
        ...
        while (true) {
            wgsLat = (mLat + pLat) / 2;
            wgsLon = (mLon + pLon) / 2;
            GPS gps = wgsToGCJ(wgsLat, wgsLon);
