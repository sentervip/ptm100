...
public class AddBabyActivity extends BaseActivity implements View.OnClickListener, HeadIconDialog.HeadActionlistener, PickDialogFragment.PickerListener {
    EditText nickName;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.add_baby_activity_layout);
        ...
        switch (v.getId()) {
            case R.id.add_btn:
                ...
                if (babySex.getText().toString() == null) {
                    ...
                    String sex_value = babySex.getText().toString();
                    if (sex_value != null && !"".equalsIgnoreCase(sex_value)) {
                        if (sex_value.equals(getResources().getString(R.string.boy))) {
                            curBaby.setSex(1);
                        } else {
                            curBaby.setSex(2);
                        }
                ...
                if (curAge != null && !"".equalsIgnoreCase(curAge)) {
                    curBaby.setBabyAge(curAge);
                }
                if (head_path != null) {
                    curBaby.setHead_url(head_path.toString());
                }
                curBaby.setIsCheck(switch_btn.isChecked() ? 1 : 0);
                Intent intent = new Intent(this, MainActivity.class);
                intent.putExtra("baby_key", curBaby);
                startActivity(intent);
                daoSession.getBabyEntityDao().insert(curBaby);
                break;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == CAMERA_RESULT) {
                if (mPhotoFile != null)
                    displayImage(mPhotoFile.getPath());
            }
        }
        if (iconDialog != null) {
            iconDialog.dismiss();
    ...
    public void openAlbum() {
        if (ContextCompat.checkSelfPermission(AddBabyActivity.this,
                Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(AddBabyActivity.this, new
    ...
    public void takePhoto() {
        ...
        mPhotoFile = FileUntil.makeSDFile(parentPath, file_path);
        if (Build.VERSION.SDK_INT >= 29) {
            photoOutputUri = PhotoUtils.createImageUri(this);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            photoOutputUri = FileProvider.getUriForFile(this, getPackageName() + ".fileprovider", mPhotoFile);
        } else {
            photoOutputUri = Uri.fromFile(mPhotoFile);
        }
